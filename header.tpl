...
{/if}
		...
		{if $debug}
			...
			{/if}
		{else}
			<meta name="fullcss" content="{$css_files|key|escape:'html':'UTF-8'}" />
			<meta name="fulljs"  content="{$js_files[0]|escape:'html':'UTF-8'}" />

			<!--#if expr="$HTTP_COOKIE=/fullcss\=true/" -->
			<link rel="stylesheet" href="{$css_files|key|escape:'html':'UTF-8'}"  />
			<!--#else -->
			{assign var="criticalcss" value="$tpl_dir./css/critical/$page_name.css"}
			{if file_exists($criticalcss)}
			<style>
				{include file="$criticalcss"}
			</style>
			{/if}
			{$js_def}
			<script>
				{include file="$tpl_dir./js/enhance.js"}
			</script>
			<noscript>
				<link rel="stylesheet" href="{$css_files|key|escape:'html':'UTF-8'}" />
			</noscript>
